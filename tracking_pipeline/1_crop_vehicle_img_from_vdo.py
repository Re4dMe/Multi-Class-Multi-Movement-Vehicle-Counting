...
out_dir = "../aic19-track1-mtmc/adjust_c_cropped_imgs2020"
...
class GtBox(object):
    def __init__(self, id, box, score, v_type):
        self.id = id
        self.box = box
        self.score = score
...
def analysis_to_frame_dict(file_path):
    ...
    for line in lines:
        ...
        score = float(words[6])
        pred = str(words[10].strip())
        cur_gt_box = GtBox(id, box, score, pred)
...
def main():
    ...
    for scene_dir in scene_dirs:
        ...
        for camera_dir in camera_dirs:
            ...
            #video_path = camera_dir + '/cam_7_rain.mp4'
            det_path = camera_dir + '/det/box.txt'
            ...
            for i in range(0, all_frames):
                ...
                if tmp_i in frame_dict:
                    ...
                    for det_box in boxes:
                        ...
                        if det_box.box[2] < 20 or det_box.box[3] < 20:
                            pass
                            #continue
                        ...
                        if not os.path.exists(out_dir+"/"+fd_list[fd_count]):
                            os.makedirs(out_dir+"/"+fd_list[fd_count])
                        out_path = os.path.join(out_dir+"/"+fd_list[fd_count], img_name)

                        cv2.imwrite(out_path, cropped_img)

                        ww = img_name + ',' + str(tmp_i) + ',-1,' + str(box[0]) + ',' + str(box[1]) + ',' + \
                             str(box[2]) + ',' + str(box[3]) + ',' + str(score) + ',-1,' + str(GPS_coor[0]) + ',' + str(
                            GPS_coor[1]) + ','  + str(det_box.pred)
                        ww += '\n'
                        out_f.write(ww)
